  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-6 sm:px-8">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="font-display text-2xl font-bold text-tc-navy">{t("path.title")}</h1>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-4 rounded-2xl border border-tc-border bg-white p-5 shadow-sm">
          <h2 className="font-display text-sm font-bold text-tc-navy">{t("path.criteria")}</h2>

          <Field label={t("field.businessUnit")}>
            <Select value={bu} onValueChange={(v) => { setBu(v); setPosition(ANY); }}>
              <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value={ANY}>{t("common.allF")}</SelectItem>
          ...
          <Field label={t("path.projectType")}>
          ...
          <div className="rounded-lg bg-slate-50 p-3 text-xs text-slate-600">
            <span className="font-semibold">{matching.length}</span> {t("path.matchingRules")} · <span className="font-semibold">{suggested.length}</span> {t("path.suggestedModules")}
          </div>
          ...
          <Button onClick={applySuggestion} disabled={suggested.length === 0}
            ...
            <Wand2 className="mr-1.5 h-3.5 w-3.5" /> {t("path.suggest")}
          </Button>
        </div>
        ...
        <div className="space-y-4 lg:col-span-2">
          <div className="rounded-2xl border border-tc-border bg-white p-5 shadow-sm">
            <h2 className="mb-3 font-display text-sm font-bold text-tc-navy">
              {t("path.modules")} ({selected.length})
            </h2>
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 border-t border-tc-border pt-4 sm:grid-cols-2">
              <div>
                <Label className="text-xs">{t("path.name")}</Label>
                <Input value={name} onChange={(e) => setName(e.target.value)}
                       placeholder={t("path.namePlaceholder")} className="mt-1 h-8 text-xs" />
              </div>
              <div>
                <Label className="text-xs">{t("path.note")}</Label>
                <Textarea value={note} onChange={(e) => setNote(e.target.value)} rows={1}
                          placeholder={t("path.notePlaceholder")} className="mt-1 text-xs" />
              </div>
            </div>
            ...
            <div className="mt-3 flex justify-end">
              <Button onClick={save} disabled={!name.trim() || selected.length === 0}
                ...
                <Save className="mr-1.5 h-3.5 w-3.5" /> {t("path.save")}
              </Button>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-tc-border bg-white p-5 shadow-sm">
            <h2 className="mb-3 font-display text-sm font-bold text-tc-navy">
              {t("path.saved")} ({trainingPaths.length})
            </h2>
                  ...
                  <div key={p.id} className="flex items-start gap-3 rounded-xl border border-tc-border p-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-[10px] text-slate-400">
                        {t("path.createdBy")} {p.createdBy} {t("path.on")} {new Date(p.createdAt).toLocaleDateString("fr-CA")}
                      </p>
                    </div>
